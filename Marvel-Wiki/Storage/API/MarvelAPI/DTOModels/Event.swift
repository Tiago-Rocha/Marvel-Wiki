//
// Event.swift
//
// Generated by swagger-codegen
// https://github.com/swagger-api/swagger-codegen
//

import Foundation



public struct Event: Codable {

    /** The unique ID of the event resource. */
    public var _id: Int?
    /** The title of the event. */
    public var title: String?
    /** A description of the event. */
    public var _description: String?
    /** The canonical URL identifier for this resource. */
    public var resourceURI: String?
    /** A set of public web site URLs for the event. */
    public var urls: [Url]?
    /** The date the resource was most recently modified. */
    public var modified: Date?
    /** The date of publication of the first issue in this event. */
    public var start: Date?
    /** The date of publication of the last issue in this event. */
    public var end: Date?
    /** The representative image for this event. */
    public var thumbnail: Image?
    /** A resource list containing the comics in this event. */
    public var comics: ComicList?
    /** A resource list containing the stories in this event. */
    public var stories: StoryList?
    /** A resource list containing the series in this event. */
    public var series: SeriesList?
    /** A resource list containing the characters which appear in this event. */
    public var characters: CharacterList?
    /** A resource list containing creators whose work appears in this event. */
    public var creators: CreatorList?
    /** A summary representation of the event which follows this event. */
    public var next: EventSummary?
    /** A summary representation of the event which preceded this event. */
    public var previous: EventSummary?

    public init(_id: Int?, title: String?, _description: String?, resourceURI: String?, urls: [Url]?, modified: Date?, start: Date?, end: Date?, thumbnail: Image?, comics: ComicList?, stories: StoryList?, series: SeriesList?, characters: CharacterList?, creators: CreatorList?, next: EventSummary?, previous: EventSummary?) {
        self._id = _id
        self.title = title
        self._description = _description
        self.resourceURI = resourceURI
        self.urls = urls
        self.modified = modified
        self.start = start
        self.end = end
        self.thumbnail = thumbnail
        self.comics = comics
        self.stories = stories
        self.series = series
        self.characters = characters
        self.creators = creators
        self.next = next
        self.previous = previous
    }

    public enum CodingKeys: String, CodingKey { 
        case _id = "id"
        case title
        case _description = "description"
        case resourceURI
        case urls
        case modified
        case start
        case end
        case thumbnail
        case comics
        case stories
        case series
        case characters
        case creators
        case next
        case previous
    }


}

